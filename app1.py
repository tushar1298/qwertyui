...
# ZIP ARCHIVE CONFIG
PDB_ZIP_NAME = "pdb_archive_01.zip"
...
def load_pdb_zip():
    """
    Downloads pdb_archive_01.zip from Supabase and extracts all PDBs into memory.
    Returns: dict {pdb_filename.lower(): pdb_text}
    """
    if not supabase:
        return {}

    try:
        zip_bytes = supabase.storage.from_(BUCKET_NAME).download(PDB_ZIP_NAME)
        pdb_map = {}
            ...
            for name in z.namelist():
                if name.lower().endswith(".pdb"):
                    with z.open(name) as f:
                        pdb_map[name.split("/")[-1].lower()] = f.read().decode("utf-8")

        return pdb_map

    except Exception as e:
        st.error(f"Failed to load PDB ZIP: {e}")
        return {}


supabase = init_supabase()

PDB_CACHE = load_pdb_zip()
...
def fetch_pdb_from_supabase(filename_or_id: str) -> str | None:
    """
    Fetch PDB text from extracted ZIP cache
    """
    if not filename_or_id:
        return None

    fname = filename_or_id.lower()
    if not fname.endswith(".pdb"):
        fname = f"{fname}.pdb"

    return PDB_CACHE.get(fname)
...
def fetch_pubchem_references(pubchem_id: str) -> list[dict]:
    """
    Fetch PubMed references for a compound using PubChem CID.
    """
    if not pubchem_id or str(pubchem_id).lower() == "nan":
        return []

    try:
        url = f"https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/{pubchem_id}/xrefs/PubMedID/JSON"
        ...
        refs = []
        for pmid in pmids[:5]:  # limit to first 5
            refs.append({
                "title": f"PubMed Article {pmid}",
                "pubmed_id": pmid,
                "journal": "PubChem / PubMed",
                "year": "",
                "doi": ""
            })
        return refs
...
def render_database():
    ...
    with tab_refs:
        ...
        if chembl_id and not refs_df.empty and 'chembl_id' in refs_df.columns:
            ref_matches = refs_df[
                refs_df['chembl_id'].astype(str).str.upper()
                == chembl_id.upper()
            ]
        ...
        if not ref_matches.empty:
            st.markdown("### References (Curated)")
            for _, ref in ref_matches.iterrows():
        ...
        if not references_rendered:
            ...
            pubchem_refs = fetch_pubchem_references(pubchem_id)

            if pubchem_refs:
                st.markdown("### References (PubChem)")
                for ref in pubchem_refs:
                    render_row(
                        "PubMed",
                        ref["pubmed_id"],
